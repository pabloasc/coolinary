...
export async function loader({ request }: LoaderArgs) {
  ...
  let recipeListItems = {};
  if (userId) {
    recipeListItems = await getRecipeListItems({ userId });
  }
...
export default function Index() {
  ...
  const removeItemsToContext = (ingredients: Ingredient[]) => {
    ...
    arrayIngredients?.forEach(
      (ingId) => (tempItems = tempItems.filter((e) => e.id !== ingId))
    );
    setContextList({
      ...contextList,
      cacheId: generateId(),
  ...
  const removeRecipeIngredients = (recipeId: string) => {
    const withRemoved = contextList.items.filter(
  ...
  const handleChangeIngredients =
    (ingredient: Ingredient, recipe: Recipe) => (e) => {
      // Destructuring
      const { checked } = e.target;
              ...
              {data?.recipeListItems?.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                  {data?.recipeListItems?.map((recipe) => (
                    <div
                      key={recipe.id}
                      className="m-2 transform cursor-pointer rounded-lg border border-gray-400 bg-white transition-all duration-200 hover:-translate-y-1 hover:border-opacity-0 hover:shadow-md"
                    >
                      <div className="card-body p-5">
                        <div className="form-control">
                          <label className="label cursor-pointer">
                            <input
                              checked={contextList.items.find(
                                (obj) => obj.recipeId === recipe.id
                              )}
                              type="checkbox"
                        ...
                        {recipe.ingredients?.map(
                          (ingredient: { id: number; description?: string }) =>
                            ...
                            !collapsed && (
                              <p
                                key={ingredient.id}
                                className="font-serif text-sm font-light text-gray-700"
                              >
                                <input
                                  type="checkbox"
                                  checked={contextList.items.find(
